...
WORD_LIST_EXCEPTIONS = {
    ...
    "ADD_TO_A_NUMBER_WORDS_KO": "the Korean list word read as arithmetic when the name holds a number; English `append`/`push` can only mean a list",
    "FOR_EACH_CLOSING_WORDS_KO": "the word that closes a Korean loop-over-a-list header (`이름마다 돌아`); English states its verb first and needs no closer",
    "EACH_MARKER_WORDS_KO": "the `each` of `for each`, written between the list and the loop name; English has it in EACH_WORDS_EN",
    "SUBTRACT_SOFT_WORDS_KO": "everyday verbs for taking one thing out of a list; the English twin is SUBTRACT_SOFT_WORDS_EN",
    "SUBTRACT_SOFT_WORDS_EN": "everyday verbs for taking one thing out of a list; the Korean twin is SUBTRACT_SOFT_WORDS_KO",
    "CONDITION_CLOSING_WORDS_KO": "the word that closes a Korean condition (`3보다 클 때`); English opens with `if`/`when` instead (WHEN_WORDS_EN)",
    "WHILE_MARKERS_KO": "the word that closes a Korean loop condition; English opens with `while` instead (WHILE_WORDS_EN)",
    "NOT_A_COUNT_KO": "everyday words that read as a number glued to a counter (`이번`); English writes its counts apart, so no English word wears that shape",
    ...
    "AS_WORDS_EN": "the `as` of `greeting as a number`; Korean marks it with the particle on `숫자로` instead",
    "CHANGED_WORDS_KO": "the `바꾼` of `숫자로 바꾼 것`; English says the whole thing with AS_WORDS_EN",
    "JOINED_THING_WORDS_KO": "`쉼표로 이은 것`, the joined list written as a thing; English reuses JOIN_WORDS_EN in both places",
    "WHOLE_WORDS_EN": "the `whole` of `the whole number of`; Korean says it in the one word QUOTIENT_WORDS_KO",
    "WHOLE_NUMBER_WORDS_EN": "the `number` of `the whole number of`; Korean says it in the one word QUOTIENT_WORDS_KO",
    "GLUED_SHORT_CONDITION_ENDINGS_KO": "Korean endings that a word may end in by accident (`황금가면`); English writes its connector as a word of its own and cannot glue one on",
...
MATRIX = [
    ...
    row("list_count_in_a_sentence", "How many items, said inside a sentence",
        ["TextPart::Reading"],
        line("show You carry how many friends things"),
        line("You carry friends 개수 things 말해줘"),
        line('say "You carry " + str(len(friends)) + " things"'),
        line('말해 "You carry " + str(len(friends)) + " things"'),
        line('print("You carry " + str(len(friends)) + " things")')),
    ...
    row("remainder", "What is left over after a division", ["Value::Remainder"],
        ...
        line("print(score % 4)")),
    row("quotient", "How many whole times one number goes into another",
        ["Value::Quotient"],
        line("show the whole number of score divided by 4"),
        line("score를 4로 나눈 몫 말해줘"),
        line("say score // 4"), line("말해 score // 4"),
        line("print(score // 4)")),
    row("as_number", "Text written in digits, read back as a number",
        ["Value::AsNumber"],
        line("show greeting as a number"),
        line("greeting을 숫자로 바꾼 것 말해줘"),
        line("say int(greeting)"), line("말해 int(greeting)"),
        line("print(int(greeting))")),
...
INVENTORY_EXCEPTIONS = {
    ...
    "ConditionValue::Quotient": "how one side of a condition is stored",
    "ConditionValue::AsNumber": "how one side of a condition is stored",
