...
**아래 5분 과정은 전부 그 화면에서 하시면 됩니다.** 설치는 필요하지 않습니다.
파일을 저장하거나 인터넷을 쓰는 프로그램이 필요해지는 날에
[설치 안내](install.ko.md)를 보시면 됩니다. 설치를 이미 하셨다면, 아래 프로그램을
`hello.nme` 같은 파일에 적고 `nme 실행 hello`로 돌려도 결과는 같습니다.

## 1. Hello World

쓰는 칸에 이렇게 적고 **실행**을 누릅니다.

```nme
안녕하세요! 말해줘
```

`안녕하세요!`가 나옵니다. 완전한 프로그램을 만들었지만 따옴표와 괄호가
하나도 없습니다.

쓴 것을 계속 두고 싶으면 쓰는 칸 위의 **파일 1·2·3** 가운데 하나로 옮겨
두세요. 그 세 파일은 브라우저에 남아 있어서, 예제를 열어 보고 돌아와도
사라지지 않습니다.

## 2. 프로그램과 대화하기

쓰는 칸의 내용을 이렇게 바꾸고 다시 **실행**을 누릅니다.
...
`점수는 0`은 숫자를 저장하고, `점수가 3보다 작은 동안`은 그 말이 참인 동안
계속하라는 뜻이며, `점수에 1 더해`는 값을 늘립니다.

같은 방식으로 `멈춰`, `그리고`/`또는`, `아니면 만약에`, `아니면`을 쓸 수
있습니다.
...
게임이 질문을 하고 답을 입력받을 때까지 기다립니다. 사이트에서는 쓰는 칸
위의 예제 가운데 **숫자 맞히기**를 누르면 완성된 것을 바로 볼 수 있습니다.
설치했다면 저장소의 예제로도 돌려 볼 수 있습니다
([한국어](../examples/guessing-game.ko.nme) ·
[영어](../examples/guessing-game.nme)).
...
## 5. 목록과 기다리기 — 여기까지도 전부 문장입니다

목록은 말하듯이 씁니다. 파일 내용을 아래로 바꿔 주세요:
...
여기까지 따옴표도, 대괄호도, 콜론도, 등호도 쓰지 않았습니다.
**문장형은 나중에 버리는 연습용이 아니라 NME 그 자체입니다.** 가이드 01~16은
처음부터 끝까지 이 문법으로만 쓰여 있고, 출력·질문·목록·반복·조건·무작위·
이야기·화면·시간 재기·쿨타임까지 다룹니다.

원하는 날이 오면 쓸 수 있는 단계가 둘 더 있습니다. 더 짧은 초급 문법과,
NME가 쓴 그대로 남겨 두는 평범한 Python입니다. 둘 다 프로그램을 완성하는 데
필요하지 않고, 설명은 [문법 안내](language.ko.md)에 있습니다. 프로그래밍을
처음 배우는 중이라면 문장형에 머무르세요. 다음 절에서, 이미 쓴 문장이 어떤
Python이 되었는지 눈으로 볼 수 있습니다.
