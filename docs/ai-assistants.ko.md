...
먼저 `nme --version`으로 지원되는 베타(현재 `0.0.1-beta.154`)를 확인하세요. 그다음 요청한 프로그램을 .nme 파일로 작성하세요. 초보자에게는 문장형을 우선하고,
...
`nonlocal`은 바깥 함수가 있어야 합니다. 최상위, 최상위 클래스, 또는
중첩되지 않은 함수에서 쓰면 `E0113`이 표시됩니다. 바깥 함수 아래의 중첩
함수와 클래스는 그대로 사용할 수 있고, 이름이 실제로 바깥 함수에 있는지는
CPython이 검사합니다.
Python 별표 import(`from ... import *`)는 모듈 수준에서만 쓸 수 있습니다.
함수나 클래스 안에서 사용하면 `E0114`가 표시되므로 이름을 명시적으로
import하세요.
...
`global`과 `nonlocal`은 같은 범위의 사용이나 대입보다 먼저 적어야 하고 매개변수
이름을 사용할 수 없습니다. 이런 충돌은 NME가 `E0119`/`E0120`으로 표시하므로
선언을 앞에 적으세요. annotation 식의 이름도 사용으로 계산하며, f-string 검증은
CPython에 맡기고 컴프리헨션의 지역 이름은 구분합니다.
