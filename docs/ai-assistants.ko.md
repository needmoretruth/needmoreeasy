...
**[needmoreeasy.com](https://needmoreeasy.com/ko/) 맨 위에 프롬프트 세 개가
있습니다.** 하나를 통째로 복사해서 쓰던 대화창 맨 앞에 붙여넣으면 그것으로
끝입니다. 그 글은 컴파일러에서 자동으로 만들어지므로 언제나 최신이고, 링크를
읽을 수 없는 대화창에서도 됩니다. 코딩을 처음 한다면 첫 번째(문장문법만)를
쓰세요.

아래는 링크를 읽을 수 있는 도구(코딩 도우미·에이전트)에게 문서 주소만 건네고
싶을 때 쓰는 방법입니다.
...
먼저 `nme --version`으로 지원되는 베타(현재 `0.0.1-beta.160`)를 확인하세요. 그다음 요청한 프로그램을 .nme 파일로 작성하세요. 초보자에게는 문장형을 우선하고,
...
같은 괄호 안에서 연결어 표기를 섞을 수도 있어
`만약 (점수가 2보다 크면 and 준비)`처럼 적을 수 있습니다.
한국어 NME 동작어도 올바른 Python 식별자일 수 있습니다. `만약`이 바인딩되어
있다면 `만약 (준비)`는 Python 호출 형태이므로 바이트 단위로 그대로 두어야
합니다. NME 블록을 뜻한다면 `만약 준비라면`처럼 문장형 끝말을 쓰거나
`만약 ((준비 그리고 참))`처럼 NME 연결어를 넣으세요.
...
사용자가 제한된 네이티브 백엔드를 요청하면 불리언 이름은 정수와 구별되는
타입으로 지원됩니다. `True`/`False`를 대입하고 참·거짓 `if`/`while` 조건에
...
`say`/`show`/`말해` 또는 `break`를 쓸 수 있고, 문장형 반복에서는 한 줄
`여기서 멈춰` 본문도 쓸 수 있습니다. Python 인라인 본문과 인라인 값 변경은
...
Python의 문맥 규칙도 그대로 적용됩니다. `return`과 `yield`는 함수 안에,
`await`는 `async def` 안에, `break`·`continue`는 반복문 안에 있어야 합니다.
잘못된 최상위·인라인·한 줄 함수/클래스 형태를 `nme 검사`하면 안정적인 이중
언어 코드 `E0102`, `E0106`–`E0110`이 표시되며 한 줄 함수나 클래스는 바깥
반복문을 물려받지 않습니다. 한 줄 본문 앞에 세미콜론으로 구분한 문장이
있어도 같은 제어 흐름 검사가 적용됩니다.
`async def`에서는 `yield from` 대신 `async for`를 사용하세요.
`async for`와 `async with`도 `async def` 안에 있어야 하며, 잘못된 위치에는
`E0111`과 `E0112`가 표시됩니다.
`nonlocal`은 바깥 함수가 있어야 합니다. 최상위, 최상위 클래스, 중첩되지 않은
함수, 한 줄 본문에서 잘못 쓰면 `E0113`이 표시됩니다. 바깥 함수 아래의 중첩
함수와 클래스는 그대로 사용할 수 있고, 이름이 실제로 바깥 함수에 있는지는
CPython이 검사합니다.
Python 별표 import(`from ... import *`)는 모듈 수준에서만 쓸 수 있습니다.
한 줄 본문이나 앞선 세미콜론 구분 문장 뒤에 함수·클래스 안에서 사용하면
`E0114`가 표시되므로 이름을 명시적으로 import하세요.
Python은 `except*` 안의 `break`, `continue`, `return`도 거부하며 NME는
핸들러 안에서 앞선 세미콜론 구분 문장 뒤에 나오는 경우에도 `E0115`를
표시합니다. 제어 흐름을 핸들러 밖으로 옮기거나 의미가 맞을 때 일반
`except` 블록을 사용하세요.
...
비동기 컴프리헨션은 보존하세요.
비동기 제너레이터에서는 값을 반환할 수 없으며 첫 `yield`보다 앞의 반환도
NME가 `E0118`로 표시합니다. 한 줄로 쓴 Python 본문도 함수 본문으로 추적하므로
올바른 인라인 `yield`, `await`, 값 없는 `return`은 그대로 남습니다. 값 없는
`return`을 사용하고, 중첩 함수의 반환은 그대로 두세요.
`global`과 `nonlocal`은 같은 범위의 사용이나 대입보다 먼저 적어야 하고 매개변수나
annotation 대상을 이름으로 사용할 수 없습니다. 한 줄 본문을 포함한 이런 충돌은
NME가 `E0119`/`E0120`으로 표시하므로 선언을 앞에 적으세요. annotation 식의
이름도 사용으로 계산하며, f-string 검증은 CPython에 맡기고 컴프리헨션의 지역
이름은 구분합니다.
