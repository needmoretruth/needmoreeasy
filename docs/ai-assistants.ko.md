...
먼저 `nme --version`으로 지원되는 베타(현재 `0.0.1-beta.157`)를 확인하세요. 그다음 요청한 프로그램을 .nme 파일로 작성하세요. 초보자에게는 문장형을 우선하고,
...
한 줄 본문을 포함해 함수나 클래스 안에서 사용하면 `E0114`가 표시되므로
이름을 명시적으로 import하세요.
Python은 `except*` 안의 `break`, `continue`, `return`도 거부하며 NME는
`E0115`를 표시합니다. 제어 흐름을 핸들러 밖으로 옮기거나 의미가 맞을 때
일반 `except` 블록을 사용하세요.
