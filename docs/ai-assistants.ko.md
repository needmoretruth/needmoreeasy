...
먼저 `nme --version`으로 지원되는 베타(현재 `0.0.1-beta.155`)를 확인하세요. 그다음 요청한 프로그램을 .nme 파일로 작성하세요. 초보자에게는 문장형을 우선하고,
...
`await`는 `async def` 안에 있어야 하며, 잘못된 최상위·인라인 형태를
`nme 검사`하면 안정적인 이중 언어 코드 `E0106`–`E0110`이 표시됩니다.
