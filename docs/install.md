...
NME `0.7.1` installs from source. You need Git,
...
Expected NME version: `0.7.1`. The `$env:Path` line makes the
...
Expected NME version: `0.7.1`. The `export` line must run before the
...
Expected NME version: `0.7.1`. The `export` line is deliberately
before `nme --version`.
...
Expected NME version: `0.7.1`. Expected random adapter: `0.0.1`.
