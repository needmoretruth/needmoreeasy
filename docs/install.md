...
NME `0.0.1-beta.121` currently installs from the beta branch. You need Git,
...
Expected NME version: `0.0.1-beta.121`. The `$env:Path` line makes the
...
Expected NME version: `0.0.1-beta.121`. The `export` line must run before the
...
Expected NME version: `0.0.1-beta.121`. The `export` line is deliberately
before `nme --version`.
...
Expected NME version: `0.0.1-beta.121`. Expected random adapter: `0.0.1`.
