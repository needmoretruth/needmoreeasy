...
NME `0.8.0` installs from source. You need Git,
...
Expected NME version: `0.8.0`. The `$env:Path` line makes the
...
Expected NME version: `0.8.0`. The `export` line must run before the
...
Expected NME version: `0.8.0`. The `export` line is deliberately
before `nme --version`.
...
Expected NME version: `0.8.0`. Expected random adapter: `0.0.1`.
