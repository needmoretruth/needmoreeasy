...
예제를 실행해 보세요. 단어 하나를 바꾸고 결과를 보는 것도 학습입니다.

**설치는 필요하지 않습니다.** [needmoreeasy.com](https://needmoreeasy.com/ko/)의
쓰는 칸에 그대로 적고 **실행**을 누르면 됩니다. 설치를 이미 하셨다면 같은
내용을 `.nme` 파일에 적고 `nme 실행 <파일이름>`으로 돌려도 결과는 같습니다.
아래에서는 명령줄을 다시 적지 않습니다.
...
필요하지 않았습니다. **넷 다 문장형만으로 끝냈습니다.** 다음 두 프로젝트는
Python을 이미 아는 사람, 또는 언젠가 알고 싶어진 사람을 위한 것입니다. 지금
넘어가도 되고, 한참 뒤에 돌아와도 됩니다 — 가이드 01~16은 계속 문장형입니다.
