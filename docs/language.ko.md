...
정수·실수·문자열의
`say`/`show`/`말해`. 실수 산술 결과가 유한하지 않게 되면 이중 언어 런타임
오류로 멈춥니다. 그 밖의 모든 것 — 입력, 모듈, 파일, 클래스,
