...
이 충돌은 고치는 방법을 안내하는 E9028로 표시됩니다. `nme compile`은 아직
모듈 가져오기를 지원하지 않아 E9029를 표시하므로, 다른 `.nme` 파일을 가져오는
프로그램은 `nme 실행`, `nme 검사`, 또는 `nme 빌드`를 사용하세요.
...
기계어로 컴파일할 수 있습니다. `nme native 실행 hello`는 시스템 C
컴파일러로 C를 만들어 실행 파일을 돌리고, `nme native 빌드 hello -o
