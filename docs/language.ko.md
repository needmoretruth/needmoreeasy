...
`for _ in range(...)`, `if`, 간단한 대입, `import random`을 바꿉니다.
...
- 문장 출력은 간단한 대입, 함수 매개변수, 단순한 Python 반복 변수, NME 입력,
  ...
  글은 초급 표현식을 쓰세요.
- 문장형 비교 단어는 의도적으로 작습니다. 복잡한 논리는 `만약 표현식:` 또는
  고급 Python을 쓰세요.
- 쉬운 모듈 문법은 이번 베타에서 내장 random 어댑터만 제공합니다.
- `검사`는 NME 토큰화와 문법을 확인하며 Python의 모든 문법·실행 오류는
  CPython이 담당합니다.
- `실행`에는 CPython이 필요합니다. `빌드`와 `검사`에는 NME만 있으면 됩니다.
  선택적인 `컴파일`에는 Python, Nuitka, 운영체제용 C 컴파일러가 필요합니다.
