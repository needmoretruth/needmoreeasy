...
- Python 함수나 클래스 안의 `from ... import *`에는 `E0114`가 표시되므로
  그 안에서는 이름을 명시적으로 import하세요. 모듈 수준의 별표 import와
...
- 같은 범위에서 이름을 먼저 사용하거나 대입한 뒤의 `global`·`nonlocal`
  선언에는 각각 `E0119`·`E0120`이 표시되며, 매개변수에는 두 선언을 사용할 수
  없습니다. 선언을 먼저 적고, annotation에서 사용한 이름도 사용으로
  계산합니다. f-string 검증은 CPython에 맡기며 올바른 모듈·중첩 함수·컴프리헨션
  범위는 보존됩니다.
