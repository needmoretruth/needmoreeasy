...
괄호로 NME 조건 전체를 감쌀 수도 있습니다. 예를 들어
`만약 (준비 그리고 점수 > 2)`처럼 씁니다. NME 블록으로 해석하려면 헤더에
콜론을 붙이지 않아야 하며, `when(ready and score > 2)`처럼 올바른 Python
호출은 바이트 단위로 그대로 Python으로 남습니다.
