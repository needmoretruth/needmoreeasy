...
- 바깥 함수가 없는 Python `nonlocal`에는 한 줄 함수·클래스 본문을 포함해
  ...
  따로 검사합니다.
- Python 함수나 클래스 안의 `from ... import *`에는 한 줄 본문을 포함해
  `E0114`가 표시되므로 그 안에서는 이름을 명시적으로 import하세요. 모듈
  수준의 별표 import와 모듈 수준 조건문 안의 사용은 그대로 보존됩니다.
- Python은 `except*` 블록 안의 `break`, `continue`, `return`을 허용하지
  않으며 NME는 `E0115`를 표시합니다. 중첩 함수 본문과 핸들러 뒤의 제어
  흐름은 그대로 보존됩니다.
