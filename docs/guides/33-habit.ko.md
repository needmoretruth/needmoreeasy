...
- 난이도: ★★★★☆ (4/5)
- 선수 지식: [32 — 성적부](32-grade-book.ko.md), [30 — 상점](30-shop.ko.md)
- 주제: 프로젝트
- 결과물: add, check, streak, list, quit를 지원하고 저장 로직을 모듈 파일로 둔 JSON 저장 습관 추적기

[35](35-todo.ko.md) 가이드는 딕셔너리 목록의 할 일을 저장하고, [61](61-modules.ko.md) 가이드는 프로젝트를 여러 파일로 나눕니다. 습관 추적기는 딕셔너리 하나 — 습관마다 연속 날수 — 를 JSON으로 저장합니다.
...
3. 파이프로 명령을 넣어 실행합니다:
   ...
   습관이 0에서 2로 자랐습니다 — `habits.json`에 `{"물 마시기": 2}`가 저장됩니다. 영어는 같은 메뉴를 `ask`와 `show`로 씁니다 — 전체 쌍은 [33-habit.md](33-habit.md)에 있습니다.
