...
1. 네이티브 코어 안에 머무는 프로그램을 쓰세요. 코어는 정수·유한 실수와
   ...
   각 함수는 모든 경로에서 조건 없이 정수를 반환하는 최상위 `return`이
   필요합니다. 같은 파일의
   ...
   밖에서 사용할 수 없습니다. `if`/`else` 사슬의 모든 분기에서 대입한 이름은
   그 뒤에 사용할 수 있지만, 한 분기에서 처음 대입한 이름을 형제 분기에서 읽을
   수도 없고 실행되지 않을 수 있는 반복문 안에서 만든 이름은 조건부로 남습니다.
