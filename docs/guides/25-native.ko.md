...
1. 네이티브 코어 안에 머무는 프로그램을 쓰세요. 코어는 정수·유한 실수와
   ...
   합니다. 같은 파일의 뒤쪽에 정의된 함수도 호출할 수 있으며 선언된 위치 인자
   개수를 맞춰야 합니다.
   헤더에는
   ...
   경로에 대입한 이름은 그 뒤에 사용할 수 있고, 일찍 반환하는 분기는 그 이름을
   대입할 필요가 없습니다. 계속되는 분기 중 한 곳에서 처음 대입한 이름을 형제
   분기에서 읽을 수도 없고 실행되지 않을 수 있는 반복문 안에서 만든 이름은
   조건부로 남습니다.
