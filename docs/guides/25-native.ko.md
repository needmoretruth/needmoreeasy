...
1. 네이티브 코어 안에 머무는 프로그램을 쓰세요. 코어는 불리언·정수·유한
   ...
   정수 `return` 함수, `say`/`show`/`말해`를 다룹니다. 네이티브 `if`/`while`과
   분기 본문은 `then`/`그러면` 뒤에 한 줄 NME 출력 문장을 이어 쓸 수도
   있습니다:
   ...
   Python 인라인 본문과 인라인 값 변경은 이 제한된 네이티브 부분집합 밖입니다.
   그런 형식에는 CPython 경로를 사용하세요.
