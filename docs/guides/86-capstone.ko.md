# 86 — 캡스톤: Python으로 컴파일하는 언어

[English](86-capstone.md) | 한국어
...
실행했습니다. [82](82-bytecode.ko.md)은 명령을 데이터 단계로 바꾸었습니다.
...
2. 컴파일러는 각 줄을 Python 한 줄로 바꿉니다. `split()`이 줄을 단어로
   ...
   Python이 필요로 하는 앞 공백을 씁니다 — [34](34-chart.ko.md)에서 막대를
   그렸던 것과 같은 문자열 곱셈입니다. 전체 컴파일러는 완성된 소스를
   `file_write`로 `out.py`에 저장하고([37](37-files.ko.md)), 파일을 다시 읽어
   `exec`로 실행합니다 — [85](85-selfhost.ko.md)의 실행기가 이제 진짜 파일을
   ...
   `print("done")`이 됩니다 — [62](62-project-files.ko.md)에서 가져왔던 이름
   목록의 아주 작은 버전입니다.
...
4. 영어는 같은 컴파일러를 `use file latest`, `show`로 씁니다. 전체 영어
   프로그램은 [영어 가이드](86-capstone.md)에 있습니다.

## 직접 해보기

`-=`로 낮추는 `sub` 동사와, `text`를 N번 출력하는 `say text N` 형식을 더해
보세요 — 반복 번역 힌트는 [85](85-selfhost.ko.md)에 있습니다. 그런 다음
