...
1. 프로그램 옆에 `story.txt`를 만들고 읽어 봅니다:
   ...
   `글자 수: 101`과 `단어 수: 20`이 출력됩니다. `파일읽기`([13](13-files.ko.md))가
...
3. 이제 전체 보고서를 한 파일에 씁니다. `텍스트통계.nme`로 저장하고 실행합니다:
   ...
   파일 하나, 질문 네 개의 답입니다. 영어 가이드는 `file_read`와 `show`로 같은 Python 호출을 씁니다 — 전체 쌍은 [41-text-stats.md](41-text-stats.md)에 있습니다.
