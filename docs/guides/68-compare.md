# 68 — Compare: two groups of numbers

English | [한국어](68-compare.ko.md)
...
1. Create and load the two data files, each holding one JSON list (the shape guide [39](39-json.md) saved with `json_save`):
...
2. Mean by hand, mean by library, and max. `sum(a) / len(a)` is the mean; `statistics.mean` (guide [58](58-data.md)) checks it; `max` is a builtin:
...
3. The comparison program. Save `compare.nme` with both JSON files, and run it:
   ...
   means. The Korean guide uses `json읽기` and `보여줘` — full pair in [68-compare.ko.md](68-compare.ko.md).
