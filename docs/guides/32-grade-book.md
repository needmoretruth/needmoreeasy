...
the shop in guide [30](30-shop.md) ran a command menu. A grade book puts both
...
`report`, and `quit`. `statistics.mean` from guide [67](67-stats.md) turns a
...
2. Storage lives in a module, `gradebook.nme`, exporting `load()` and `save`,
   exactly like the bank module in guide [31](31-bank.md). `load()` returns an
...
guide [46](46-top-ten.md). Or make `report` print the highest grade too with
`max(scores)`.
