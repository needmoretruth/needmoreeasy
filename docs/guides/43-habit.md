...
3. Run it and feed the commands through a pipe:
   ...
   The habit started at 0 and grew to 2 — `habits.json` holds `{"water": 2}`. Korean writes the same menu with `물어봐` and `말해` — full pair in [43-habit.ko.md](43-habit.ko.md).
