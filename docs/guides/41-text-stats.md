...
English | [한국어](41-text-stats.ko.md)
...
1. Create `story.txt` next to your program and read it back:
   ...
   [13](13-files.md)) returns the text; `.split()` cuts the word list.
...
3. Now the whole report in one file. Save `text-stats.nme` and run it:
   ...
   One file, four questions answered. The Korean guide uses `파일읽기` and `보여줘` with the same Python calls — full pair in [41-text-stats.ko.md](41-text-stats.ko.md).
