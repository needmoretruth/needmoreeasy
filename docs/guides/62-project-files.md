# 62 — Modules: a project across files

English | [한국어](62-project-files.ko.md)
...
[32](32-grade-book.md) used one storage module. A real project is bigger than
...
2. `fetch.nme` is the input module. Its only job is to read the file; the
   `use file` module from guide [39](39-json.md) does the reading:
...
3. `analyze.nme` is the brains. `average` adds the temperatures and divides,
   like guide [67](67-stats.md); `hottest` walks the days with an `if`,
...
8. Korean writes the same project with `파일 사용 최신`, `json읽기`, `말해`, and
   ...
   [Korean guide](62-project-files.ko.md).
