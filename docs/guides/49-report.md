# 49 — Report — writing a summary file

English | [한국어](49-report.ko.md)
...
1. Create two data files. `scores.json` pairs each player with a score, and
   ...
   [24](24-quiz.md). The `name` keys line up, which is how the two files stay
   about the same players.

2. Load both files with `json_load` from guide [39](39-json.md). Two calls,
   two lists:
...
3. Walk the scores with a `for` loop to total them and find the top scorer. The
   ...
   This is the running-total and running-max pattern from guide [58](58-data.md),
...
5. Build the report as a list of lines and join them with `"\n".join(lines)`.
   ...
   helper that saved each diary note in guide [36](36-diary.md):
   ...
   numbers in guide [52](52-sorting.md).
...
8. Korean uses `파일 사용 최신`, `json읽기`, and `파일쓰기`. The full Korean
   program is in the [Korean guide](49-report.ko.md).
