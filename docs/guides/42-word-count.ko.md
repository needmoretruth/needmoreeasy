...
- 난이도: ★★★★☆ (4/5)
- 선수 지식: [38 — 이름 목록](38-name-list.ko.md), [37 — 파일](37-files.ko.md)
- 주제: 목록과 글
...
2. 파일 전체를 읽고 단어로 나눕니다. `파일읽기`가 글을 돌려주고
   ([37](37-files.ko.md) 가이드) `.split()`이 빈칸마다 잘라냅니다:
...
3. dict로 직접 셉니다. 빈 dict `{}`와 `for` 반복 하나: 단어가 이미 있으면
   ...
   `셈표`가 단어마다 개수를 담습니다. [38](38-name-list.ko.md) 가이드는
   목록을 썼습니다. dict는 이름이 붙은 칸이 있는 목록입니다.

4. `collections.Counter`가 한 줄로 같은 셈을 하고 `most_common(5)`로 가장
   흔한 다섯 단어를 더해 줍니다. [64](64-python-packages.ko.md) 가이드의
   `date` 가져오기처럼 불러옵니다:
