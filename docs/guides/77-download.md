...
3. `urlopen` gives you a response object. Its `.read()` call returns the
   ...
   same call that decoded JSON in guide [32](32-weather.md).
...
6. A big download should not vanish into silence. The response can be read
   ...
   server in guide [76](76-poll.md), but here the server sends until it is
   done.
...
download every file it finds — guide [65](65-files-folder.md) lists a
folder, and each file name becomes one `urlopen` call. Or download a page
that changes (like `status.json` in guide [76](76-poll.md)) and save one
snapshot per poll.
