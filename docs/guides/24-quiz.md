...
adds to a score. This guide keeps the questions in a JSON file, so changing the
quiz means editing data, not code; guide [39](39-json.md) covers that file
format in full. The
menu loop is the one from guide [18](18-adventure.md), aimed at options instead
of directions.
...
2. Load the questions with `json_load` from guide [39](39-json.md). The file
