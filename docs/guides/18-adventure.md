...
- Difficulty: ★★★★☆ (4/5)
- Prerequisites: [12 — Random](12-random.md), [08 — If](08-if.md)
...
3. The game world is one endless loop with a small menu. `while True:` never
   stops by itself, so the only way out of a room is answering `quit` or
   winning:
