# 58 — Data: statistics on a list

English | [한국어](58-data.ko.md)
...
1. Create the data file `numbers.json` with one JSON list:
   ...
   Six numbers stored as a JSON list. Guide [39](39-json.md) saved a dict with
...
2. Load the list with `use file latest` and `json_load`. The loaded value is a
   ...
   list, `json_load` returns a list rather than the dict from guide [39](39-json.md).

3. Import `mean` and `median` from the standard library. Guide
   [64](64-python-packages.md) uses `from datetime import date`; importing
...
6. Korean writes the same steps with `파일 사용 최신` and `json읽기`. The full
   Korean program is in the [Korean guide](58-data.ko.md); this snippet loads
   the list:
