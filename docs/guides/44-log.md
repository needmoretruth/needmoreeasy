# 44 — Log: an event record

English | [한국어](44-log.ko.md)
...
1. Ask `datetime` for now and shape it as text. `datetime.now()` is this
   moment; `strftime` (guide [64](64-python-packages.md)) formats it — `%Y`
...
2. `file_write` from guide [37](37-files.md) replaces the whole file, so
...
the dict counting from guide [42](42-word-count.md) for how many lines share
each date.
