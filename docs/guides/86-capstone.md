# 86 — Capstone: a language that compiles to Python

English | [한국어](86-capstone.ko.md)
...
memory. Guide [82](82-bytecode.md) turned instructions into data steps. The
...
2. The compiler turns each line into one Python line. `split()` splits a line
   ...
   multiplication that drew the bars in guide [34](34-chart.md). The full
   compiler also writes the finished source to `out.py` with `file_write`
   (guide [37](37-files.md)) and runs it by reading the file back with `exec`
   — guide [85](85-selfhost.md)'s runner, now reading a real file. Save
   `capstone.nme`:
...
4. Korean writes the same compiler with `파일 사용 최신`, `말해`, and Korean
   ...
   program is in the [Korean guide](86-capstone.ko.md).

## Try it yourself

Add a `sub` verb that lowers to `-=`, and a `say text N` form that prints
`text` N times — guide [85](85-selfhost.md) hinted at the loop translation.
