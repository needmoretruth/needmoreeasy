...
English | [한국어](26-tic-tac-toe.ko.md)
...
## Steps
1. The whole game in one file, `tictactoe.nme`. The board and `show_board` come from [55](55-grid.md); `check_winner` scans rows, columns, and both diagonals and returns the winning mark, or `None` while no line is complete; `ask number` reads a row and a column as integers, `continue` re-asks a taken square, and `moves == 9` declares a draw:
   ```nme
   # tictactoe.nme — a two-player tic-tac-toe game.
   # Run: nme r tictactoe
   # The board is a list of rows (guide 55). A turn places a mark,
