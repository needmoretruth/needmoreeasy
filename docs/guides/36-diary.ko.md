...
2. 메모를 날짜가 들어간 파일에 저장합니다. `f"..."` 안의 `f`가 `{오늘}`을
   파일 이름에 채워 넣고, [37](37-files.ko.md) 가이드의 `파일쓰기`와
