...
[English](48-files-folder.md) | 한국어

[README](../../README.ko.md) | [설치](../install.ko.md) | [시작하기](../getting-started.ko.md) | [학습 과정](../tutorial.ko.md) | [문법 안내](../language.ko.md) | [가이드](index.ko.md)

- 난이도: ★★★★★ (5/5)
- 선수 지식: [37 — 파일](37-files.ko.md), [42 — 단어 세기](42-word-count.ko.md)
- 주제: 파일
- 결과물: 폴더의 파일을 나열해 각각 읽고 전체 단어·글자 수를 보고하는 프로그램

[37](37-files.ko.md) 가이드는 코드에 이름을 적은 파일 하나를 읽었습니다.
실제 폴더에는 파일이 많고, 미리 이름을 알지 못하는 경우가 보통입니다.
`os.listdir(".")`은 폴더의 모든 이름을 목록으로 돌려주고,
[37](37-files.ko.md)의 `파일읽기`는 가리키는 파일을 읽습니다. 둘이 만나면
...
3. `.endswith(".txt")`가 텍스트 파일만 남깁니다. `for` 반복이 남은 이름을
   `파일읽기`로 하나씩 읽고 [42](42-word-count.ko.md)처럼 단어로
   쪼갭니다:
...
5. 전체 프로그램은 먼저 `.txt` 이름을 모으고 파일마다 보고한 뒤 전체
   ...
   [23](23-high-score.ko.md)의 최고 점수와 같은 "지금까지 중 최고" 생각입니다.
...
7. 영어는 같은 단계를 `import os`, `use file latest`, `file_read`로 씁니다.
   전체 영어 프로그램은 [영어 가이드](48-files-folder.md)에 있습니다.
