...
현재 NME `0.0.1-beta.149`는 beta 브랜치의 소스에서 설치합니다. Git, Cargo가
...
4. PowerShell을 닫았다 다시 열고 실행합니다. `nme 네이티브`(영어 `nme native`)를 사용하려면
   `cl.exe`가 `PATH`에 들어 있는 Visual Studio Developer PowerShell을 여세요.
...
NME 버전은 `0.0.1-beta.149`가 표시되어야 합니다. `$env:Path` 줄은 방금 설치한
...
NME 버전은 `0.0.1-beta.149`가 표시되어야 합니다. Cargo가 바이너리 폴더가
...
NME 버전은 `0.0.1-beta.149`가 표시되어야 합니다. `export` 줄을 의도적으로 첫
`nme` 명령보다 앞에 두었습니다.
...
NME는 `0.0.1-beta.149`, 랜덤 어댑터는 `0.0.1`이 표시되어야 합니다.
