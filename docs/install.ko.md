...
현재 NME `0.0.1-beta.47`는 beta 브랜치의 소스에서 설치합니다. Git, Cargo가
...
NME 버전은 `0.0.1-beta.47`가 표시되어야 합니다. `$env:Path` 줄은 방금 설치한
...
NME 버전은 `0.0.1-beta.47`가 표시되어야 합니다. Cargo가 바이너리 폴더가
...
NME 버전은 `0.0.1-beta.47`가 표시되어야 합니다. `export` 줄을 의도적으로 첫
`nme` 명령보다 앞에 두었습니다.
...
NME는 `0.0.1-beta.47`, 랜덤 어댑터는 `0.0.1`이 표시되어야 합니다.
