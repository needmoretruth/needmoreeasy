...
| 문장형 | `한줄은 친구들을 쉼표로 이은 것` | `한줄 = ", ".join(map(str, 친구들))` |
| 문장형 | `레벨은 레벨글을 숫자로 바꾼 것` | `레벨 = int(레벨글)` |
...
| 문장형 | `점수를 4로 나눈 몫 말해줘` | `print(점수 // 4)` |
| 문장형 | `줄수는 점수를 인원으로 나눈 몫` | `줄수 = 점수 // 인원` |
| 문장형 | `줄수는 점수를 4로 나눈 몫` | `줄수 = 점수 // 4` |
