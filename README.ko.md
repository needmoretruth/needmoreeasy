...
> ### [needmoreeasy.com](https://needmoreeasy.com/ko/) · [nmelang.com](https://nmelang.com/ko/)
>
> **설치할 것도 없고, 깃허브를 알 필요도 없습니다.** 두 주소 모두 같은 사이트로
> 갑니다. 컴파일러와 파이썬 실행기가 브라우저 안에서 돌기 때문에, 쓰는 칸에 한
> 문장을 적고 실행을 누르면 됩니다. 컴퓨터에서도 휴대폰에서도 됩니다. 가이드
> 90편과 이 문서의 모든 예제, 문법 목록 전부를 그 사이트에서 평범한 웹 문서로
> 읽을 수 있습니다.
...
표시될 버전은 `nme 0.0.1-beta.160`입니다.
...
## 버전이 붙은 랜덤·파일·영지식·목록·글자·수학·날짜 도구
...
도구 상자 네 개가 버전 `0.0.1`로 더 들어 있습니다. `list` / `목록`
(`개수`/`count`, `정렬`/`sort`, `뒤집기`/`reverse`, `빼기`/`remove`,
`첫번째`/`first`, `마지막`/`last`, `합계`/`sum`, `최대`/`largest`,
`최소`/`smallest`), `text` / `글자` (`대문자`/`upper`, `소문자`/`lower`,
`공백없애기`/`trim`, `나누기`/`split`, `합치기`/`join`, `바꾸기`/`replace`,
`로시작`/`starts_with`, `길이`/`length`), `math` / `수학` (`제곱근`/`root`,
`반올림`/`round_to`, `원주율`/`pi`, `거듭제곱`/`power`, `절댓값`/`absolute`,
`내림`/`floor`, `올림`/`ceil`), `date` / `날짜` (`오늘`/`today`,
`지금`/`now`, `올해`/`year`, `이번달`/`month`, `오늘일자`/`day_of_month`,
`요일`/`weekday`, `며칠뒤`/`days_after`)입니다.
...
안에 든 것은 전부 평범한 Python 기본 기능이거나 `math`·`datetime` 호출
하나라서 브라우저에서도 그대로 돕니다. 이름이 평범한 낱말이라서, `사용`/`use`
바로 옆에 있고 그 줄에 다른 낱말이 남지 않을 때만 모듈로 읽습니다.
`장 볼 목록을 사용해 보세요`와 `이 날짜 사용법을 알려 주세요`는 문장이고 그대로
출력됩니다.

날짜 모듈은 한국어 이름과 영어 이름이 서로 다른 값을 갖는 유일한 자리입니다.
요일 이름은 숫자가 아니라 낱말이기 때문입니다. 수요일이면 `요일()`은 `수요일`을,
`weekday()`는 `Wednesday`를 돌려줍니다. 시계는 그 기계의 시계이고, 브라우저가
Python에 주는 시계는 UTC라서 브라우저 연습장에서 `오늘()`은 UTC 기준의
오늘입니다.
...
- [네이티브 코어 레퍼런스](docs/native-reference.ko.md) — v0 값·문장·함수의 정확한 범위와 여섯 표면 예제
...
**직접 쓴 예제.** NME로 만든 것 중에 다른 사람이 읽거나 돌려 보거나 즐길 만한
것이 있으면 `examples/`에 둘 수 있습니다. 프로그램과 함께 그것이 무엇을 보여
주는지 한 문단으로 적어 풀 리퀘스트를 열어 주세요. 여기에 들어오는 것은 모두
이 저장소의 나머지와 같은 Apache-2.0을 따르므로, 그 조건으로 내놓을 수 있는
것만 보내 주시기 바랍니다. 저장소가 쓰는 점검표는
[좋은 NME 예제 쓰는 법](docs/guides/example-authoring.ko.md)에 있습니다.

**컴파일러 자체.** 받아 줬어야 했는데 막힌 표기, 도움이 되지 않은 오류 문장,
자꾸 쓰게 되는 줄임말, 그 언어에서는 자연스러운데 아직 거절당하는 문장 — 전부
풀 리퀘스트로 보낼 만한 것입니다. 기여자가 따르는 설계 규칙은
[컴파일러 구조](docs/architecture.md)에 있습니다(영어 문서).

**아이디어만 남겨도 됩니다.** 코드를 쓰지 않아도 괜찮습니다. *「이렇게 썼는데
될 줄 알았습니다」* 라고 적은 이슈 하나가 이 저장소가 받을 수 있는 가장 값진
것 중 하나입니다. 그 문장이 바로 이 언어가 받아들이려 애쓰는 것이기 때문입니다.
예제 아이디어도 똑같이 환영합니다.
